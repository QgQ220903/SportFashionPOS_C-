    ...
    public partial class PhieuNhapModule : Form
    {
        ...
        public bool KiemTraNhapHangTrunng(ChiTietSanPham chiTietSanPham)
        {
            for (int i = 0; i < danhSachChiTietPhieuNhap.RowCount; i++)
            {
                if ( danhSachChiTietPhieuNhap.Rows[i].Cells[0].Value.ToString() == chiTietSanPham.MaChiTietSanPham.ToString() 
                   && danhSachChiTietPhieuNhap.Rows[i].Cells[1].Value.ToString() == mauSacBUS.LayMauSacQuaMa(chiTietSanPham.MaMauSac).TenMauSac
                   && danhSachChiTietPhieuNhap.Rows[i].Cells[2].Value.ToString() == kichCoBUS.LayKichCoQuaMa(chiTietSanPham.MaKichCo).TenKichCo )
                {
                    int soLuongHienTai = Convert.ToInt32(danhSachChiTietPhieuNhap.Rows[i].Cells[3].Value);
                    soLuongHienTai += Convert.ToInt32(slNhap.Value);
                    danhSachChiTietPhieuNhap.Rows[i].Cells[3].Value = soLuongHienTai;
                    return true;
                }
            }
            return false;
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if (KiemTraNhapHangTrunng(chiTietSanPham))
                {
